...
namespace IFluent.Движок.Сущности
{
    internal class Игра
    {
        ...
        {
            Имя = описаниеИгры.СвойствоИмя;
            Локации = описаниеИгры.СписокЛокаций;
            Настройки = описаниеИгры.СущностьНастройки;
        }
        ...
        {
            public ОписаниеИгры Имя(Текст имя)
            {
                СвойствоИмя = имя;
                return this;
            }

            internal Текст СвойствоИмя { get; private set; }
...
*/
                ...
                */
                ...
                internal class ОписаниеЛокации
                {
                    ...
                    public ОписаниеЛокации(ОписаниеЛокаций описаниеЛокаций, Текст название,
                        Текст описание)
                    {
            ...
            {
                private readonly ОписаниеИгры _описаниеИгры;
                public Локация СвойствоНачатьВЛокации { get; private set; }
                ...
                {
                    Локация локация = _описаниеИгры.СписокЛокаций.Find(лок =>
                        ((string)лок.Название).Equals(названиеЛокации,
                            StringComparison.OrdinalIgnoreCase));

                    СвойствоНачатьВЛокации = локация ?? throw new ОшибкаОписания($"Настройка \"НачатьВЛокации\" ссылается на несуществующую локацию \"{названиеЛокации}\".");
                ...
                {
                    _описаниеИгры.СущностьНастройки = new Настройки(this);
                    return _описаниеИгры;
                }

                public ОписаниеНастроек НачатьВЛокации(Локация локация)
                {
                    СвойствоНачатьВЛокации = локация;
                    return this;
                }
            }

            public Настройки СущностьНастройки { get; private set; }
        }
        ...
        public void ЗапуститьИгру()
        {
            ...
            Текст списокЛокаций =
                Локации.Select(локация =>
                    {
                        string списокКнопок = локация.Кнопки
                            .Select(кнопка =>$"\"{кнопка.Надпись}\" => {кнопка.Локация?.Название}")
                            .Join(", ");
            ...
            ОжидатьКлавишу("Игра завершена. Нажмите любую клавишу для выхода.");
        }
        ...
        private void ЗапуститьЦиклИгры()
        {
            ...
            do
            {
                ...
                var списокКнопок = текущаяЛокация.Кнопки.Select((кнопка, номер) => new
                    ...
                    .ToList();
                Вывести(списокКнопок.Select(кнопка => кнопка.Текст).Join("\n"));
                ...
                do
                {
                    ...
                    клавишаПодходит = этоЧисло && клавиша >= 1 &&
                                      клавиша <= текущаяЛокация.Кнопки.Count;
                } while (!клавишаПодходит);
                if (вышли)
                {
                    break;
                }
                текущаяЛокация = списокКнопок.Single(кнопкаЛоки => кнопкаЛоки.Номер == клавиша).Локация;
            } while (true);
        }
